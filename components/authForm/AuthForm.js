...
export default function AuthForm({ 
  ...
  useEffect(() => {    
    if (status === "authenticated") {
      push(`${process.env.BASE_URL}/admin`)
    }
  }, [status])
  ...
  */
  ...
  const handleSubmit = async (e) => {
    ...
    if (typeAuth == "signin") {
      ...
      try {        
        res = await axios({
          url: `${process.env.BASE_URL}/api/auth/users/signup`,
          method: "post",
          data: fields
        })

        if (res.status == 201) {
          push(`${process.env.BASE_URL}/auth/verify/${res.data._id}`)
        }
        ...
        <Card className="bg-secondary shadow border-0">
          <CardBody className="px-lg-5 pt-lg-5">
            <div className="text-center text-muted mb-4">
              <small>{typeAuth == "signup" ? "Sign up" : "Sign in"} with credentials</small>
            </div>
            <Form role="form" onSubmit={handleSubmit}>
              ...
              <div className="text-center">
                <Button 
                  ...
                  type="submit"
                >
                  { typeAuth == "signup" ? "Create account" : "Sign In"}
                </Button>
              </div>
            </Form>
          </CardBody>
          <CardHeader className="bg-transparent pb-5">
            <div className="text-muted text-center mt-1 mb-3">
              <small>Or sign { typeAuth == "signup" ? "up" : "in" } with</small>
            </div>
            <div className="text-center">
              <Button
          ...
          (
            <Row className="mt-3">
              <Col className="text-center" xs="12">
                <a
                  ...
                  href={`${process.env.BASE_URL}/auth/signin`}
                >
            ...
            <Row className="mt-3">
              ...
              <Col className="text-right" xs="6">
                <a
                  className="text-light"
                  href={`${process.env.BASE_URL}/auth/new-user`}
                >
